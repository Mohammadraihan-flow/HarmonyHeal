...
document.getElementById('getBtn').addEventListener('click', async () => {
  ...
  document.getElementById('output').innerHTML = '<p>Loading recommendation...</p>';
  ...
  try {
    ...
    if (videos.length === 0) html += '<p>No videos found. Check backend YT API key.</p>';
...
function loadHistory() {
  ...
  ul.innerHTML = hist.map(h => `<li>${h.time}: ${h.mood} → ${h.freq} Hz</li>`).join('');
}
...
${text}
    ...
    console.error(e);
    document.getElementById('output').innerHTML = '<p>Failed to load history.</p>';
  }
});
